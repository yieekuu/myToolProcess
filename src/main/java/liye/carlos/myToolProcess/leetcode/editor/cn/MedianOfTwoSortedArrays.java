...
public class MedianOfTwoSortedArrays {
    ...
    class Solution {
        ...
        private int getKth1(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
            ...
            int len2 = end2 - start2 + 1; //让 len1 的长度小于 len2，这样就能保证如果有数组空了，一定是 len1
            ...
            if (nums1[i] > nums2[j]) {
                return getKth(nums1, start1, end1, nums2, j, end2, k - (j - start2));
            } else {
                return getKth(nums1, i, end1, nums2, start2, end2, k - (i - start1));
            }
        }
